...
router.post('/trips/search', (req, res) => {
    ...
    request
        ...
        .end(function (err, response) {
            ...
            for (i=0; i < result.trips.data.carrier.length; i++){
                 fees = db.get('fees').filter((element) => {
                    return element.code.toLowerCase().indexOf(result.trips.data.carrier[i].code.toLowerCase()) > -1
                }).value();
                if (fees.length > 0){
                    result.trips.data.carrier[i]['fees'] = fees[0];
                    result.trips.data.carrier[i]['fees']['numBags'] = req.body.request.bags;
                    result.trips.data.carrier[i]['fees']['numCarryon'] = req.body.request.carryon;
                }
            }
            console.log(req)
